...
def sync_organic_in_app_events():

    schema = load_schema("raw_data/organic_in_app_events")
    singer.write_schema("organic_in_app_events", schema, [
        "event_time",
        "event_name",
        "appsflyer_id"
    ])
    ...
    stop_time = datetime.datetime.now(pytz.utc)
    from_datetime = get_start("organic_in_app_events")
    to_datetime = get_stop(from_datetime, stop_time, 10)
    ...
    while from_datetime < stop_time:
        ...
        url = get_url("organic_in_app_events", app_id=CONFIG["app_id"])
        request_data = request(url, params)
        ...
        for i, row in enumerate(reader):
            record = xform(row, schema)
            singer.write_record("organic_in_app_events", record)
            # AppsFlyer returns records in order of most recent first.
            if utils.strptime_to_utc(record["event_time"]) > bookmark:
                bookmark = utils.strptime_to_utc(record["event_time"])

        # Write out state
        utils.update_state(STATE, "organic_in_app_events", bookmark)
        singer.write_state(STATE)
